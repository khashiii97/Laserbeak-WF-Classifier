        ...
        with tqdm(dataloader, desc=desc, 
            ...
            for batch_idx, (inputs, targets, sample_sizes) in enumerate(pbar):
                ...
                if return_raw:
                    ...
                    all_targets = np.concatenate((all_targets, targets.numpy(force=True)))
    ...
    if eval_only:
        if resumed:
            ...
            if include_unm:
                ...
                thresholds = np.concatenate((thresholds, np.linspace(0.9, 1.0, num=10, endpoint=False)))
            ...
            train_loss, train_acc, raw_res = epoch_iter(trainloader, 
                                                            ...
                                                            return_raw=True)
            if include_unm:
                res = calc_ow(raw_res[0], raw_res[1], raw_res[2], 
                             thresholds = thresholds, print_res = True)

            # validation eval
            if valloader is not None:
                val_loss, val_acc, raw_res = epoch_iter(valloader, 
                                                            desc = "Val.",
                                                            eval_only=True, 
                                                            return_raw=True)
                if include_unm:
            ...
            if testloader is not None:
                test_loss, test_acc, raw_res = epoch_iter(testloader, 
                                                            desc = "Test",
                                                            eval_only=True, 
                                                            return_raw=True)
                if include_unm:
